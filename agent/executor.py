...
def _run_generated_code(description: str, speak: Callable | None = None) -> str:
    ...
    if speak:
        speak("Writing some custom code — give me a sec.")
...
def _step_failed(result: str) -> bool:
    r = (result or "").strip()
    if not r or r in ("Done.", "Completed."):
        return False
    markers = (
        "FAILED:",
        "Path not found:",
        ...
        "Could not copy:",
        "Merge incomplete",
        "will not delete a non-empty folder",
    )
...
def _call_tool(tool: str, parameters: dict, speak: Callable | None) -> str:
    ...
    if tool == "open_app":
        ...
        return send_email(parameters=parameters, player=None) or "Done."

    elif tool == "contact_manager":
        from actions.contacts import contact_manager
        return contact_manager(parameters=parameters, player=None) or "Done."

    elif tool == "calendar_control":
        from actions.calendar import calendar_control
        return calendar_control(parameters=parameters, player=None) or "Done."

    elif tool == "reminder":
        from actions.reminder import reminder
        return reminder(parameters=parameters, player=None) or "Done."

    elif tool == "notes_control":
        from actions.notes import notes_control
        return notes_control(parameters=parameters, player=None) or "Done."

    elif tool == "organizer_control":
        from actions.organizer import organizer_control
        return organizer_control(parameters=parameters, player=None) or "Done."

    elif tool == "document_tools":
        from actions.document_tools import document_tools
        return document_tools(parameters=parameters, player=None) or "Done."

    elif tool == "list_manager":
        from actions.list_manager import list_manager
        return list_manager(parameters=parameters, player=None) or "Done."

    elif tool == "screen_act":
        from actions.screen_act import screen_act
        return screen_act(parameters=parameters, player=None) or "Done."
...
class AgentExecutor:
    ...
    def execute(
        ...
        while True:
            ...
            if not steps:
                msg = "I couldn't map out a plan for this one."
                if speak: speak(msg)
                return msg
            ...
            for step in steps:
                if cancel_flag and cancel_flag.is_set():
                    if speak: speak("Task cancelled — no worries.")
                ...
                while attempt <= 3:
                    ...
                    try:
                        ...
                        if decision == ErrorDecision.RETRY:
                            ...
                            msg = f"Stopping here. {recovery.get('reason', '')}"
                            if speak: speak(msg)
                            return msg
                            ...
                            if fix_suggestion and tool != "generated_code":
                                try:
                                    ...
                                    if speak: speak("Trying a different angle — bear with me.")
                                    res = _call_tool(
                                        fixed_step["tool"],
            ...
            if replan_attempts >= self.MAX_REPLAN_ATTEMPTS:
                msg = f"Hit the wall after {replan_attempts} replans — want to narrow the goal?"
                if speak: speak(msg)
                return msg

            if speak: speak("Pivoting — new plan incoming.")

            replan_attempts += 1
            plan = replan(goal, completed_steps, failed_step, failed_error)

    def _summarize(self, goal: str, completed_steps: list, speak: Callable | None) -> str:
        fallback = f"All done — {len(completed_steps)} steps knocked out for: {goal[:60]}."
        try:
            from core.llm import ask
            ...
            summary = ask(prompt, model="gemini-2.5-flash-lite")
