...
- Return ONLY the JSON, no explanation, no markdown."""
    ...
    try:
        return ask_json(prompt, model="gemini-2.5-flash-lite")
    except Exception as e:
...
def computer_settings(
    ...
    # Fast local routing for volume/brightness (no API, no PyAutoGUI)
    if description or raw_action:
        from actions.system_control import resolve_command_from_text, system_control

        hint = f"{raw_action} {description}".strip()
        quick = resolve_command_from_text(hint)
        if quick:
            return system_control({"command": quick}, player=player)
    ...
    if not _PYAUTOGUI and action not in _NO_PYAUTOGUI_OK and _OS != "Darwin":
        return "pyautogui is not installed. Run: pip install pyautogui"
    if not _PYAUTOGUI and action in _NO_PYAUTOGUI_OK:
        from actions.system_control import system_control

        return system_control({"command": action}, player=player)
