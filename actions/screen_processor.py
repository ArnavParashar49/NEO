...
def _live_voice_from_config() -> str:
    return str(_load_config().get("live_voice", "Kore")).strip() or "Kore"
...
_SYSTEM_PROMPT = (
    "You are ARIA — warm, witty, affirming. Analyze the image clearly. "
    "Two to four sentences unless the user needs more. "
    "Sound human: light humor or a pun only if it fits naturally. "
...
def _capture_screen() -> tuple[bytes, str]:
    """Capture primary display; macOS uses screencapture when available."""
    import platform
    import subprocess
    import tempfile
    ...
    if platform.system() == "Darwin":
        try:
            tmp = Path(tempfile.gettempdir()) / f"aria_screen_{int(time.time())}.png"
            r = subprocess.run(
            ...
            print(f"[Vision] ⚠️  screencapture fallback: {e}")

    if not _MSS:
        raise RuntimeError(
            "Screen capture unavailable. Install mss (pip install mss) or on Mac "
            "grant Screen Recording permission to Terminal/Python."
        )
            ...
            speech_config=gtypes.SpeechConfig(
                voice_config=gtypes.VoiceConfig(
                    prebuilt_voice_config=gtypes.PrebuiltVoiceConfig(
                        voice_name=_live_voice_from_config()
                    )
                )
            ),
        )
            ...
            async for response in self._session.receive():
                ...
                if sc.turn_complete:
                    if transcript and self._player:
                        ...
                        if full:
                            self._player.write_log(f"Aria: {full}")
                            print(f"[Vision] 💬 {full}")
                    transcript = []
...
def screen_process(
    ...
    """
    Capture screen/camera and answer via screen_act (same voice session as ARIA).
    Returns explanation text for the live model to speak.
    """
    params    = parameters or {}
    user_text = (params.get("text") or params.get("user_text") or "").strip()
    angle     = (params.get("angle") or "screen").lower().strip()

    if angle == "screen" and user_text:
        if re.search(
    ...
    if not user_text:
        return "What should I look for on your screen?"
    ...
    try:
        ...
        if result and not str(result).startswith("FAILED"):
            ...
            print(f"[Vision] ✅ {str(result)[:120]}")
            return str(result)
        return result or "I could not analyze the screen."
    except Exception as e:
        print(f"[Vision] ❌ screen_process: {e}")
        return (
            f"Screen capture failed: {e}. On Mac, open System Settings → "
            "Privacy & Security → Screen Recording and allow Terminal or your Python app."
        )
