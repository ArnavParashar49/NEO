...
def _resolve_save_path(output_path: str, language: str) -> Path:
    ...
    return DESKTOP / f"aria_code{ext}"
...
Code:"""
    ...
    response = ask(prompt, model=GEMINI_MODEL)
...
Explanation:"""
    ...
    try:
        return ask(prompt, model=GEMINI_MODEL)
    except Exception as e:
...
def _scaffold_action(description, language, output_path, player) -> str:
    """Add a single file to an existing project folder."""
    if not description:
        return "Please describe the file to add, sir."
    if not output_path:
        return "Please provide output_path as the project folder or full file path, sir."

    path = Path(output_path)
    if path.is_dir():
        ext_map = {"html": "index.html", "css": "styles.css", "javascript": "script.js", "python": "main.py"}
        fname = ext_map.get((language or "html").lower(), "new_file.txt")
        path = path / fname

    lang = language or path.suffix.lstrip(".") or "html"
    try:
        code, _ = _write(description, lang, str(path), player)
        return f"Scaffolded file at {path}\n\nPreview:\n{_preview(code)}"
    except Exception as e:
...
def code_helper(
    ...
    if action == "write":
        ...
        return _scaffold_action(description, language, output_path or file_path, player)

    else:
        return f"Unknown action: '{action}'. Use write, edit, explain, run, build, scaffold, optimize, or screen_debug."
